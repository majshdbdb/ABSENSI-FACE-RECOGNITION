...
// ============ HELPER: HITUNG HARI KERJA (Senin-Jumat) DALAM RENTANG ============
// Pendekatan sederhana -- belum memperhitungkan hari libur nasional/libur sekolah,
// itu jelas dicatat sebagai catatan di UI supaya gak disalahpahami sebagai angka final.
...
// ============ REKAP PERIODE (rentang tanggal, buat halaman "Rekap & Unduh") ============
app.get('/api/attendance/rekap', requireAuth, async (req, res) => {
  try {
    ...
    const rows = await dbAll(
      `
      SELECT a.id, a.tipe, a.nama, a.nis, a.nisn, a.id_pegawai, a.kelas_jabatan,
             COUNT(t.id) as jumlah_hadir
      FROM anggota a
      LEFT JOIN attendance t ON a.id = t.anggota_id AND t.attendance_date BETWEEN ? AND ?
      GROUP BY a.id
      ORDER BY a.tipe, a.kelas_jabatan, a.nama
    `,
      [start, end]
    ...
    res.json({ data: rows, workingDays: countWorkingDays(start, end), start, end });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});

// ============ EXPORT EXCEL (2 sheet: Rekap Ringkasan + Detail Absensi) ============
app.get('/api/attendance/export', requireAuth, async (req, res) => {
  try {
    ...
    const summaryRows = await dbAll(
      `
      SELECT a.tipe, a.nama, a.nis, a.nisn, a.id_pegawai, a.kelas_jabatan, COUNT(t.id) as jumlah_hadir
      FROM anggota a
      LEFT JOIN attendance t ON a.id = t.anggota_id AND t.attendance_date BETWEEN ? AND ?
      GROUP BY a.id
      ORDER BY a.tipe, a.kelas_jabatan, a.nama
    ...
    const detailRows = await dbAll(
      `
      SELECT a.tipe, a.nama, a.nis, a.nisn, a.id_pegawai, a.kelas_jabatan, t.attendance_date, t.time_in, t.confidence
      FROM attendance t
      JOIN anggota a ON a.id = t.anggota_id
      WHERE t.attendance_date BETWEEN ? AND ?
      ORDER BY t.attendance_date, a.tipe, a.nama
    `,
      [start, end]
    );
    ...
    workbook.creator = 'Sistem Absensi SMK Tamansiswa Mojoagung';
    workbook.created = new Date();

    const headerStyle = { font: { bold: true, color: { argb: 'FFFFFFFF' } }, fill: { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF6B1E23' } } };

    const sheet1 = workbook.addWorksheet('Rekap Ringkasan');
    sheet1.columns = [
      { header: 'Tipe', key: 'tipe', width: 12 },
      { header: 'Nama', key: 'nama', width: 28 },
      { header: 'NISN / ID Pegawai', key: 'idnum', width: 18 },
      { header: 'Kelas / Jabatan', key: 'kelas', width: 18 },
      { header: 'Jumlah Hadir', key: 'hadir', width: 14 },
      { header: 'Hari Kerja (Sen-Jum)', key: 'workingDays', width: 18 },
    ...
    summaryRows.forEach((r) => {
      sheet1.addRow({
        tipe: r.tipe,
        nama: r.nama,
        idnum: (r.tipe === 'siswa' ? r.nisn : r.id_pegawai) || '-',
        kelas: r.kelas_jabatan,
        hadir: r.jumlah_hadir,
        workingDays,
        persen: workingDays > 0 ? ((r.jumlah_hadir / workingDays) * 100).toFixed(1) + '%' : '-',
      });
    });

    const sheet2 = workbook.addWorksheet('Detail Absensi');
    sheet2.columns = [
      ...
      { header: 'NISN / ID Pegawai', key: 'idnum', width: 18 },
      { header: 'Kelas / Jabatan', key: 'kelas', width: 18 },
      { header: 'Jam Masuk', key: 'jam', width: 12 },
    ...
    detailRows.forEach((r) => {
      sheet2.addRow({
        ...
        kelas: r.kelas_jabatan,
        jam: r.time_in,
        confidence: r.confidence != null ? Number(r.confidence).toFixed(1) + '%' : '-',
      });
    });

    const filename = `Rekap_Absensi_${start}_${end}.xlsx`;
